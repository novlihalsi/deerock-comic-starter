# Episode 02 · Page 4 — Interupsi Pagi

**Save as:** `../results/ep02_page04_v001.png` · **Layout:** `2-INSET` · **Panels:** 2 · **Bubbles:** 0 — SFX and screen text only

**Attach:**

```
characters/deerock.png
locations/krl-interior.png
episodes/02-pr-review-jalur-rangkasbitung/results/ep02_page01_v001.png   ← style reference: anchor page (use the approved version)
episodes/02-pr-review-jalur-rangkasbitung/results/ep02_page03_v001.png   ← style reference: previous page (use the approved version)
```

**Canary:** the page shows no title. If "Interupsi Pagi" appears anywhere, this prompt did not run as
written. Check that the inset is a separate small panel with its own border, then Kai's message
letter by letter — it is the longest string on the page.

---

```
STYLE REFERENCE: the attached earlier comic pages are for rendering style only — match their line weight, colouring, shading and how each character's face is drawn. Do not copy their panels, their layout, their page border, their scenes, their outfits or any of their text. Clothing follows the CAST section below.

A single 4:5 portrait comic page (1080 x 1350) with 2 panels: one large panel filling the page and one small inset panel.
The canvas is exactly 4:5 — width to height 4 to 5. Not 3:4, not 2:3.
Contemporary slice-of-life manga/anime: clean linework, restrained cel shading, subtle halftone
texture, soft cinematic lighting, realistic modern Indonesian environments, natural expressions.
Muted cool palette with subtle purple and blue accents. Thin dark panel borders, with thin white gutters
BETWEEN panels only. FULL BLEED: the outer panels run right to the edges of the canvas — no white
margin, border or frame around the page. Consistent line weight, eye size and face proportion across
every character on the page. Speech and thought bubbles are compact and never cover a face.

=== CAST — match the attached character sheets exactly ===

DEEROCK — 25, male, frontend developer, 172 cm, slim. Dark PURPLE-black hair, MEDIUM length and
layered, soft strands falling over his forehead, COVERING HIS EARS and reaching the nape — it must
read purple, not black. ALWAYS wears thin dark rectangular glasses, in every panel. Youthful slim
face. CLEAN CHIN — no beard, no moustache, no stubble, no jaw shadow; the only facial hair is a
very thin, tiny, sparse patch directly beneath the lower lip.
Wears his OFFICE-CASUAL outfit from the sheet: an open BLACK OVERSHIRT worn loose over a plain
SLATE-GREY tee, dark grey chinos, dark sneakers. NO hoodie today. No harness, no straps, no pouches.
His only bag is a dark practical backpack.
Expression register: small and contained — slight frown, small smile, flat stare. Never broad.
On this page: backpack worn on his FRONT. Over-ear headphones ON HIS HEAD — never around his neck.
No lanyard.

Deerock is the only named character on this page, among anonymous commuters. Kai appears only as
the sender name on the phone screen.

ANONYMOUS COMMUTERS — ordinary Jakarta office workers and students: shirts, blouses, hoodies,
some women in hijab, some people in face masks, backpacks and tote bags. Varied faces, patient and
a little tired. None of them resembles Deerock: nobody else has purple-black hair with thin dark
rectangular glasses.

=== SETS — match the attached location sheets exactly. Do not redesign any place. ===

KRL CARRIAGE, MORNING RUSH — commuter train interior: blue patterned bench seats along the walls,
silver vertical poles, yellow and grey triangular hanging straps, overhead luggage rack, door area
with a route map. PEAK MORNING RUSH HOUR: every seat taken and the aisle packed shoulder to
shoulder with standing commuters pressed close together. Commuters wear their backpacks on the
FRONT, as KRL etiquette asks. Real rush hour, not a cartoon: patient, ordinary people.

=== PAGE LAYOUT — 4:5 portrait, one large panel with an inset (2-INSET) ===
Panel 1 fills the whole page and runs to all four edges of the canvas. Panel 2 is a small
rectangular inset panel, about one third of the page width, set a little in from the
lower-right corner, with a thin dark border and a thin white outline. The inset covers no face
and no speech bubble in Panel 1. Read Panel 1 first, then the inset.

=== PANELS ===

PANEL 1 (main panel, the whole page) — 07:45. KRL CARRIAGE, MORNING RUSH. Medium close-up from the
front. Deerock fills the upper left of the page, so the inset in the lower-right corner covers
neither his face nor his phone. His eyes have just snapped open under his headphones — a small,
sharp, alert reaction, not a cartoon shock. His phone is pinned flat against the backpack on his
chest, its screen facing UP at him, lit; its cool glow falls on his face and glasses. His arms are
pinned by the crowd — he cannot lift the phone. The commuters around him barely react. Cool window
daylight.
Small sound effect beside the phone: TING

PANEL 2 (the inset, lower-right) — a straight-on close-up insert of the phone screen, filling the
inset: a simple chat notification with a small round avatar.
Sender name: Kai
Message: Deerock, tolong review PR dong. Urgent, mau di-merge sebelum jam 9.

=== TEXT POLICY ===
The ONLY text anywhere on this page is: the sound effect "TING" in the main panel; the sender
name "Kai" and the message in the inset. There are no speech or thought bubbles on this page.
Spell every word exactly as written. Nothing else is written anywhere.

=== NEGATIVE ===
No katana, no sword, no ninja, no samurai, no fantasy armour, no tactical harness, no chest rig,
no utility straps or buckle pouches, no supernatural effects, no action aura, no glowing weapon,
no cyberpunk elements, no magazine-cover layout, no lens flare, no dramatic hero angles,
no photorealism, no chibi, no 3D render. No white border, outer margin or frame around the page.
No sweat drops, no shock lines, no cartoon symbols.
Deerock is never without glasses; his hair is never plain black; his chin is always clean —
no beard, no moustache, no stubble. Deerock wears no hoodie in this episode — only the open black
overshirt over the slate-grey tee.
Apart from the inset — a close-up insert of the screen itself — no phone screen is turned toward the camera. In the main panel the phone's screen faces Deerock.
The crowd is real rush hour, not a cartoon: no squashed or distorted faces, nobody pressed against the glass, no violent shoving, no angry faces.
Deerock's headphones are on his head, never around his neck. He wears no lanyard.
Do not redesign any room or station.
No page title, no chapter heading, no page number, no episode title, no captions, no narration
boxes, no slogans, no taglines, no character name tags, no role labels, no watermark, no
branding, no invented screen text, no readable desk nameplates, no katakana or Japanese
decoration on clothing. Do not copy any decorative text printed on the reference sheets.
```
